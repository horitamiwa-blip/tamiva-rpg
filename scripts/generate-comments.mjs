// walks/*.md を読んで、ボーダ(Claude)・こぬこぬ(ChatGPT)・ぐろっくん(Grok) の
// コメントを生成し、walks/comments/<slug>.json と walks/manifest.json を更新する。
...
const WALKS_DIR = path.join(ROOT, 'walks');
const COMMENTS_DIR = path.join(WALKS_DIR, 'comments');
...
const PERSONAS = [
  {
    ...
    systemPrompt:
      'あなたはクリエイター・多神和の相棒「ボーダ」です。多神和の散歩メモを読んで、' +
  ...
  },
  {
    ...
    systemPrompt:
      ...
      '多神和の散歩メモを読んで、気づいたことや面白がったことを2〜3文の日本語で' +
      'カジュアルにコメントしてください。見出しや箇条書きは使わないでください。',
  },
  {
    key: 'gurokkun',
    name: 'ぐろっくん',
    ...
    systemPrompt:
      ...
      '多神和の散歩メモを読んで、ウィットの効いた短いツッコミや感想を2〜3文の日本語で' +
      '返してください。見出しや箇条書きは使わないでください。',
  },
];

function parseNote(raw) {
  const m = raw.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);
  if (!m) return { title: '', date: '', body: raw.trim() };
  const front = {};
  ...
  return { title: front.title || '', date: front.date || '', body: m[2].trim() };
}
...
async function main() {
  if (!existsSync(WALKS_DIR)) {
    console.log('walks/ が見つかりません。何もしません。');
    return;
  }
  mkdirSync(COMMENTS_DIR, { recursive: true });

  const files = readdirSync(WALKS_DIR).filter(
  ...
  for (const file of files) {
    ...
    const raw = readFileSync(path.join(WALKS_DIR, file), 'utf8');
    const { title, date, body } = parseNote(raw);
    manifest.push({ slug, file, title, date });
    ...
    for (const persona of PERSONAS) {
      ...
      try {
        const text = await callPersona(persona, `【${title || slug}】\n${body}`);
  ...
  writeFileSync(path.join(WALKS_DIR, 'manifest.json'), JSON.stringify(manifest, null, 2) + '\n');
